...
At higher concurrency, a complete leaf batch becomes a drain barrier: Burner stops refilling agent slots, lets in-flight work finish, and then cooks the composite before dispatching more leaves. Short git-metadata operations wait in a lock queue instead of failing agents during simultaneous worktree startup. Command-backed evaluations with the same evaluation ID are serialized across candidates, keeping benchmark processes from racing each other while prompt evaluations and author/reviewer work can still run concurrently.
