...
export function App() {
  ...
  const mergeAnchor = dashboard.state.orchestrator.lastMergeAt ?? dashboard.state.orchestrator.mergeWindowStartedAt;
  const mergeCadenceOverdue = Boolean(yoloPortfolio && mergeAnchor && Date.now() - new Date(mergeAnchor).getTime() >= dashboard.state.settings.mergeCadenceMinutes * 60_000);
  return (
    <div className="app-shell">
      <aside className={`sidebar ${sidebar ? "sidebar-open" : ""}`}>
        ...
        <div className="sidebar-bottom">
          ...
          {yoloPortfolio && <div className="health-line"><span className={`health-dot ${mergeCadenceOverdue ? "warn" : "on"}`} /> Merge cadence {mergeCadenceOverdue ? "overdue" : `${dashboard.state.settings.mergeCadenceMinutes}m`}</div>}
      ...
      <main>
        ...
        <div className="page">
          ...
          {mergeCadenceOverdue && <div className="error-banner"><Clock3 size={16} /><span><strong>Merge cadence missed.</strong> Burner is shortening the next batch, allowing a single reviewed leaf fallback, and quarantining bounded-review blockers.</span></div>}
...
function Composites({ dashboard, busy, action, onCreate }: { dashboard: DashboardPayload; busy?: string; action: (key: string, path: string, body?: unknown) => Promise<void>; onCreate: () => void }) {
  ...
  const eligible = dashboard.state.agentRuns.filter((run) => run.prUrl && !run.leafPr?.terminal && (!run.prState || run.prState === "open")).length;
  ...
  return <>
    ...
    <div className="master-note"><Sparkles size={18} /><div><strong>{yoloPortfolio ? "A cadence-bounded PR portfolio" : "A living virtual main"}</strong><p>{yoloPortfolio ? `Burner targets a merge every ${dashboard.state.settings.mergeCadenceMinutes} minutes: it prefers ${yoloBatchSize}-leaf composites, shortens late batches, quarantines review blockers, and can merge one reviewed leaf as a final fallback.` : "New experiments branch from the selected living line. Regression-free wins are absorbed, then the entire composite is rebuilt, reviewed, and evaluated again."}</p></div></div>
    <div className="queue-summary">
...
function CompositeDialog({ dashboard, onClose, onSaved, setError }: { dashboard: DashboardPayload; onClose: () => void; onSaved: () => Promise<void>; setError: (value?: string) => void }) {
  const candidates = dashboard.state.agentRuns.filter((run) => run.prUrl && !run.leafPr?.terminal && (!run.prState || run.prState === "open"));
...
function IdeaCard({ idea, run, action }: { idea: Idea; run?: AgentRun; action: (key: string, path: string, body?: unknown) => Promise<void> }) { return <article className="idea-card"><div className="idea-top"><span className="impact-badge"><Zap size={12} /> {idea.predictedImpact.toFixed(0)}</span><span className="idea-source">{idea.source}</span></div><h3>{idea.title}</h3><p>{idea.description}</p>{idea.resources.length > 0 && <div className="lock-chips">{idea.resources.map((resource) => <span key={resource}><LockKeyhole size={11} /> {resource}</span>)}</div>}{run?.quarantinedAt && <div className="review-chip"><X size={12} /> {run.quarantineReason?.startsWith("Review yielded") ? "Yielded to preserve merge cadence" : "Quarantined after bounded review"}</div>}{run && run.reviewRounds.length > 0 && <div className="review-chip"><ShieldCheck size={12} /> {run.reviewApproved ? `Approved in ${run.reviewRounds.length} round${run.reviewRounds.length === 1 ? "" : "s"}` : `Review round ${run.reviewRounds.length}`}</div>}<div className="idea-foot"><span>{timeAgo(idea.createdAt)}</span>{run?.prUrl ? <a href={run.prUrl} target="_blank" rel="noreferrer"><GitPullRequest size={14} /> PR #{run.prNumber}</a> : run?.status === "failed" && run.worktree && run.authorThreadId ? <button onClick={() => void action(`retry-${run.id}`, `/agents/${run.id}/retry`)}><RotateCcw size={13} /> Resume run</button> : idea.status === "failed" || idea.status === "dismissed" ? <button onClick={() => void action(`retry-${idea.id}`, `/ideas/${idea.id}/status`, { status: "queued" })}><RotateCcw size={13} /> Retry fresh</button> : idea.status === "queued" ? <button onClick={() => void action(`dismiss-${idea.id}`, `/ideas/${idea.id}/status`, { status: "dismissed" })}>Dismiss</button> : <span>{run?.status.replace("_", " ")}</span>}</div>{run?.error && <div className="idea-error">{run.error}</div>}</article>; }
